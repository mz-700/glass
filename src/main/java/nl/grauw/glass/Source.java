...
public class Source {
	...
	public Expression resolve(Expression address) {
		...
		for (Line line : lines)
		{
			...
			if (line.getInstructionObject() instanceof nl.grauw.glass.instructions.Phase.PhaseObject) {
				...
				nl.grauw.glass.instructions.Phase.PhaseObject phaseObject =
					(nl.grauw.glass.instructions.Phase.PhaseObject)line.getInstructionObject();
				phaseSizeLimit = phaseObject.hasSize() ? phaseObject.getSizeLimit() : null;
				checkPhaseRange(phaseStartAddress, phaseSizeLimit, line);
				phaseLine = line;
			} else if (line.getInstructionObject() instanceof nl.grauw.glass.instructions.Dephase.DephaseObject) {
				if (phaseAddress == null) {
				...
				phaseSizeLimit = null;
				phaseLine = null;
			} else if (line.getInstructionObject() instanceof nl.grauw.glass.instructions.Org.Org_N) {
				if (phaseAddress != null) {
					phaseAddress = nextAddress;
